...
impl PaperExecutor {
    ...
    pub fn load_state(&mut self) -> Result<(), anyhow::Error> {
        if let Ok(contents) = std::fs::read_to_string(&self.state_file) {
            if let Ok(account) = serde_json::from_str(&contents) {
    ...
    pub fn buy(&mut self, market_id: &str, token_id: &str, side: OrderSide, price: Decimal, shares: Decimal) -> Result<(), String> {
        ...
        if cost > self.account.balance {
            return Err(format!("Saldo tidak cukup: butuh ${:.2}, tersisa ${:.2}", cost, self.account.balance));
        }
        ...
        if let Some(pos) = self.account.positions.get_mut(&key) {
            ...
            self.account.positions.insert(key, PaperPosition {
    ...
    pub fn sell(&mut self, market_id: &str, token_id: &str, side: OrderSide, price: Decimal, shares: Decimal) -> Result<(), String> {
        ...
        if pos.shares < shares {
            return Err("Shares tidak mencukupi".to_string());
        }
        pos.shares -= shares;
        ...
        if pos.shares == Decimal::ZERO {
            self.account.positions.remove(&key);
        } else {
            pos.entry_value = pos.avg_price * pos.shares;
        }
        self.save_state();
    ...
    pub fn get_positions(&self) -> &HashMap<String, PaperPosition> {
