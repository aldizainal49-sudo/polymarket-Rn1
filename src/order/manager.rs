...
use crate::client::types::{Order, OrderSide, OrderStatus};
...
impl OrderManager {
    ...
    pub fn new_paper(clob_client: Arc<ClobClient>, paper_executor: PaperExecutor) -> Self {
        Self {
            clob_client,
            paper_executor: Some(paper_executor),
            active_orders: HashMap::new()
        }
    }
    ...
    pub async fn place_order(&mut self, token_id: &str, side: OrderSide, price: Decimal, size: Decimal, post_only: bool) -> Result<Order, anyhow::Error> {
        if let Some(paper) = &mut self.paper_executor {
            ...
            match side {
                OrderSide::Buy => {
                    paper.buy(&market_id, token_id, side, price, size).map_err(|e| anyhow::anyhow!(e))?;
                }
                OrderSide::Sell => {
                    paper.sell(&market_id, token_id, side, price, size).map_err(|e| anyhow::anyhow!(e))?;
                }
            }
            let dummy = Order {
                order_id: format!("paper_{}_{}", token_id, chrono::Utc::now().timestamp()),
                market_id,
    ...
    pub async fn cancel_all(&mut self) -> Result<(), anyhow::Error> {
        if self.paper_executor.is_some() {
    ...
    pub async fn cancel_order(&mut self, order_id: &str) -> Result<(), anyhow::Error> {
        if self.paper_executor.is_some() {
            self.active_orders.remove(order_id);
            return Ok(());
        }
        // For real orders, we would need a cancel_order endpoint
