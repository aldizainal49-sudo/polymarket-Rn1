...
impl BacktestEngine {
    ...
    pub async fn run(&mut self, data_path: &str) -> Result<()> {
        ...
        for market in markets {
            if let Some(signal) = self.mispricing_engine.calculate_ev(&market) {
            ...
            if market.resolved {
                if let Some((side, size, avg_price)) = self.positions.remove(&market.market_id) {
                    let payout = if market.outcome_yes { size * Decimal::ONE } else { Decimal::ZERO };
                    let pnl = payout - (avg_price * size);
                    self.balance += payout;
                    if let Some(last) = self.trades.last_mut() { last.pnl = pnl; }
    ...
    fn print_summary(&self) {
        ...
        info!("   Wins:            {}", win_count);
        if total > 0 { info!("   Win Rate:        {:.2}%", (win_count as f64 / total as f64) * 100.0); }
        info!("==============================");
    }
}
